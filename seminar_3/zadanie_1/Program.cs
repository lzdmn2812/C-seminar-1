...
int enterCoordInate(string massage)
{
    Sconsole.WriteLine(massage);
...
int checkValidate(int x, int y)
{
    if (x == 0 || y == 0)
    {
        Console.WriteLine("Координата Х или У равно нулю");
        return false;
    }
    return true;
}
...
int x =enterCoordInate("Введите координату х: ");
int y =enterCoordInate("Введите координату у: ");

if  (checkValidate(x, y))
{
    Console.WriteLine("Номер четверти равен: "+ checkCategory(x,y ));
}
...
int enterCoordinate(string message){ // Ввод переменных.
...
bool checkValidate(int x, int y){ // проверку ввода пользователя что x и y не равен 0.
    if(x == 0 || y == 0){ // если x = 0 и y = 0, || - или.
...
int checkCategory(int x, int y){ // за определение категории, где находится координата.
    if(x > 0 && y > 0){ // определяем номер четверти.
        return 1;
    }
